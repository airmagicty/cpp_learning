...
int main () {
	...
	if (stringTestFirst != "11111111") {
		FileLogsEdit.open(FileNameLogsEdit);
		FileLogsEdit <<    "11111111";
		FileLogsEdit.close();
		first = true;
	}
...
//		system("pause");
	...
	cout << "Датабаза имен - Сортировка готова " << endl;
...
//	bool er1 = true;
	...
	if (first == false) {
		...
		// Айди превращаем в день
//		for (int i_all_conv = 0; i_all_conv < databaseLogsEdit.size(); i_all_conv++) {
//			for (int i_conv = 0; i_conv < dbBG_day; i_conv++) {
//				if (databaseBaseGenerate_int[0][i_conv] == databaseLogsEdit.at(i_all_conv)) databaseLogsEdit.at(i_all_conv) = i_conv;
//			}
//		}
	}
...
//		if (output_i % 36 == 0) cout << "+";
	...
	// Конец
	cout << "Операция выполнена. " << first << " " << databaseLogsEdit.at(0);
